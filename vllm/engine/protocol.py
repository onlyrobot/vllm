...
class EngineClient(ABC):
    ...
    async def beam_search(
        ...
        get_allowed_token_ids = params.get_allowed_token_ids
        ...
        beam_search_params.output_kind = RequestOutputKind.RAW
        all_beams = [
        ...
        for _ in range(max_tokens):
            ...
            for i, individual_prompt in enumerate(prompts_batch):
                ...
                if get_allowed_token_ids:
                    beam_search_params_ = beam_search_params.clone()
                    beam_search_params_.allowed_token_ids = get_allowed_token_ids(
                        individual_prompt["prompt_token_ids"], 
                        tokenized_length, tokenizer.eos_token_id
                    )
                else: beam_search_params_ = beam_search_params
                task = asyncio.create_task(
                    collect_from_async_generator(
                        self.generate(individual_prompt, beam_search_params_,
                                      request_id_item)))
                tasks.append(task)
            ...
            for i, result in enumerate(output):
                ...
                if result.outputs[0].logprobs is not None:
                    ...
                    all_beams_token_id.extend(logprobs.logprob_token_ids[0][1:])
                    all_beams_logprob.extend([
                        current_beam.cum_logprob + obj
                        for obj in logprobs.logprobs[0][1:]
                    ])
